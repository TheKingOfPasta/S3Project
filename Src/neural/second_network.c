...
Matrix* feedforward(Network* n, Matrix* a)
{
    ...
    char f = 0;
    for (Node_m *b = n->biases->head, *w = n->weights->head; b
            && w; b = b->next, w = w->next) {
...
void update_mini_batch(Network* n, Tuple_m* mini_batch, size_t len, double eta)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        for (Node_m* dnb = delta_nabla_b->head; dnb
                && nb; dnb = dnb->next, nb = nb->next) {
            Matrix* add = Add_m(nb->m, dnb->m);
            free_m(nb->m);
            nb->m = add;
        }
        Node_m* nw = nabla_w->head;
        for (Node_m* dnw = delta_nabla_w->head; dnw
                && nw; dnw = dnw->next, nw = nw->next) {
