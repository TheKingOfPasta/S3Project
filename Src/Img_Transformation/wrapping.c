...
double FindAngle(Quadrilateral* quad,int width, int height)
{
    ...
    quad->p1.x = (int)((p1.x-width/2) * cosine + (p1.y - height/2) * sine) +
                 width/2;
    quad->p1.y = (int)((p1.y - height/2) * cosine - (p1.x-width/2) * sine) +
                 height/2;
    quad->p2.x = (int)((p2.x - width / 2) * cosine + (p2.y - height / 2) * sine) +
                 width / 2;
    quad->p2.y = (int)((p2.y - height / 2) * cosine - (p2.x - width / 2) * sine) +
                 height / 2;
    quad->p3.x = (int)((p3.x - width / 2) * cosine + (p3.y - height / 2) * sine) +
                 width / 2;
    quad->p3.y = (int)((p3.y - height / 2) * cosine - (p3.x - width / 2) * sine) +
                 height / 2;
    quad->p4.x = (int)((p4.x - width / 2) * cosine + (p4.y - height / 2) * sine) +
                 width / 2;
    quad->p4.y = (int)((p4.y - height / 2) * cosine - (p4.x - width / 2) * sine) +
                 height / 2;
...
SDL_Surface* Wrapping(char* path, Quadrilateral* quad,int width, int height)
{
    ...
    SDL_Surface* wrapped = SDL_CreateRGBSurface(0,quad->p4.x-quad->p1.x,
                           quad->p2.y-quad->p1.y,32,0,0,0,0);
...
{
    SDL_Surface* wrapped = SDL_CreateRGBSurface(0, quad->p2.x - quad->p1.x,
                           quad->p4.y - quad->p1.y, 32, 0, 0, 0, 0);
    Uint32* newPixels = wrapped->pixels;
